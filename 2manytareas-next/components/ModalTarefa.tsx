...
export default function ModalTarefa({ isOpen, onClose, usuarioId, taskToEdit, onSaved }: ModalTarefaProps) {
  ...
  return (
    <div className="fixed inset-0 flex items-center justify-center bg-black/50 z-50">
      <div className="bg-slate-900 text-slate-100 rounded-2xl p-6 w-full max-w-md relative">
        <button
          onClick={onClose}
        ...
        <h2 className="text-lg font-semibold mb-4">{taskToEdit ? 'Editar Tarefa' : 'Nova Tarefa'}</h2>
        <div className="flex flex-col gap-3">
          <input
            ...
            className="border rounded p-2 bg-slate-800 text-slate-100"
          />
          <textarea
          ...
          <button
            onClick={handleSave}
            className="mt-3 w-full bg-emerald-500 text-white rounded p-2 hover:bg-emerald-600 transition"
          >
            Salvar
          </button>
        </div>
